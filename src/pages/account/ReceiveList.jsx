    ...
    <div className="premium-card">
      <div className="premium-body" style={{ padding: '32px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            <h2 style={{ fontSize: '22px', fontWeight: '700', color: 'var(--text-main)', margin: 0 }}>{t("Receive List (Deposits)")}</h2>
            <span style={{ fontSize: '13px', color: '#64748b' }}>{t("Live customer payments and deposit records")}</span>
          </div>
        ...
        <form onSubmit={handleFilter} className="filter-grid" style={{ gridTemplateColumns: '1.5fr 1fr 1fr auto', gap: '16px', marginBottom: '20px', alignItems: 'end' }}>
          <div>
            <label className="filter-label">{t("Search Reference or Client")}</label>
            <input 
              type="text" 
        ...
        <div className="table-header-controls" style={{ marginBottom: '16px' }}>
          <div className="show-entries">
            {t("Total Records:")} <strong>{receives.length}</strong>
          </div>
          <div className="table-controls-right">
            <button className="btn-blue" onClick={() => window.print()}><Printer size={16} /> {t('common.print')}</button>
            <button className="btn-blue" onClick={fetchReceives}><RotateCcw size={16} /> {t("Reload")}</button>
          </div>
        </div>
        ...
        <div className="table-responsive">
          <table className="custom-table" style={{ width: '100%' }}>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="8" style={{ textAlign: 'center', padding: '24px', color: '#64748b' }}>{t("Loading live receives...")}</td>
                </tr>
              ) : receives.length === 0 ? (
                <tr>
                  <td colSpan="8" style={{ textAlign: 'center', padding: '24px', color: '#64748b' }}>{t("No receives found for the criteria.")}</td>
                </tr>
              ) : (
                receives.map((row, idx) => (
                  <tr key={row.id || idx} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    ...
                    <td><span style={{ background: '#dbeafe', color: '#1e40af', padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold' }}>{row.transaction_type || t("Deposit")}</span></td>
                    <td style={{ fontWeight: '500' }}>{row.client_name || row.client_id || (row.client ? t("Client #{{v0}}", { v0: row.client }) : t("General / Walk-in"))}</td>
                    <td>{row.account_name || row.account || t("Main Cash")}</td>
