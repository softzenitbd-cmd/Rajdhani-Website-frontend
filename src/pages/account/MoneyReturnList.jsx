...
const MoneyReturnList = () => {
  const toast = useToast();
  const confirm = useConfirm();
  const { t } = useTranslation();
  const navigate = useNavigate();

  const [moneyReturns, setMoneyReturns] = useState([]);
  ...
  // Edit Modal State
  const [editingMoneyReturn, setEditingMoneyReturn] = useState(null);
  ...
  const fetchMoneyReturns = async () => {
    try {
      ...
      // Filter strictly for Money Returns on the frontend
      data = data.filter((row) => String(row.transaction_type || '').toLowerCase() === 'money return');

      setTotalCount(data.length); // Update total count after frontend filtering

      // Backend has no client filter on moneyReturns → filter on the client side
      if (clientId) {
      ...
      setMoneyReturns(data);
    } catch (error) {
      console.error('Error fetching moneyReturns:', error);
      setMoneyReturns([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const timer = setTimeout(() => fetchMoneyReturns(), 300);
  ...
  const handleDelete = async (id) => {
    const isConfirmed = await confirm({
      title: t("Delete MoneyReturn"),
      message: t("Are you sure you want to delete this moneyReturn? This action cannot be undone."),
    ...
    if (isConfirmed) {
      try {
        ...
        toast.success(t("MoneyReturn deleted successfully"));
        fetchMoneyReturns();
      } catch (error) {
        console.error(error);
        toast.error(t("Failed to delete moneyReturn"));
      }
    }
  };

  const startEdit = (row) => {
    setEditingMoneyReturn(row);
  };
    ...
    <div style={{ background: 'white', minHeight: '100vh', padding: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={() => fetchMoneyReturns()} style={{ background: '#3b82f6', color: 'white', border: 'none', padding: '6px 16px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontWeight: 'bold' }}>
            ...
            {loading ? (
              ...
              moneyReturns.map((row, idx) => {
                ...
                const typeStyle = (() => {
                  ...
                  if (s.includes('খরচ') || s.includes('moneyReturn') || s.includes('cost')) return { bg: '#ef4444', color: '#ffffff' };
                  ...
                  <tr key={row.id || idx} style={{ borderBottom: '1px solid #cbd5e1', textAlign: 'center' }}>
                    ...
                    <td style={cell}>
                      <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                        <button onClick={() => startEdit(row)} style={{ background: '#0ea5e9', color: 'white', border: 'none', padding: '6px 10px', borderRadius: '4px', cursor: 'pointer' }} title={t("Edit MoneyReturn")}>
                          <Edit size={16} />
                        </button>
                        <button onClick={() => handleDelete(row.id || row.uuid)} style={{ background: '#ef4444', color: 'white', border: 'none', padding: '6px 10px', borderRadius: '4px', cursor: 'pointer' }} title={t("Delete MoneyReturn")}>
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
      ...
      {/* Edit MoneyReturn Modal Popup */}
      <ExpenseEditModal
        isOpen={Boolean(editingMoneyReturn)}
        expense={editingMoneyReturn}
        onClose={() => setEditingMoneyReturn(null)}
        onSuccess={fetchMoneyReturns}
      />
    </div>
  );
};

export default MoneyReturnList;
