...
const ExpenseCategory = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      setSubmitting(true);
      if (editingCategory) {
        await accountingService.updateExpenseCategory(editingCategory.id, { name: categoryName.trim() });
  ...
  const handleDelete = async (cat) => {
    const isConfirmed = await confirm({
      ...
      message: t("Are you sure you want to delete this expense category: {{v0}}?", { v0: cat.name || '' }),
    ...
    try {
      await accountingService.deleteExpenseCategory(cat.id);
          ...
          <div className="table-responsive">
            <table className="custom-table" style={{ width: '100%', fontSize: 'var(--fs-13, 13px)', textAlign: 'center' }}>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="4" style={{ padding: '24px', textAlign: 'center', color: '#64748b' }}>{t("Loading expense categories...")}</td>
                  </tr>
                ) : categories.length === 0 ? (
                  <tr>
          ...
          <div style={{ background: 'white', borderRadius: '12px', padding: '28px', width: '100%', maxWidth: '440px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ margin: 0, fontSize: 'var(--fs-18, 18px)', fontWeight: '700', color: '#1e293b' }}>
                {editingCategory ? t("Edit Expense Category") : t("Add Expense Category")}
              </h3>
