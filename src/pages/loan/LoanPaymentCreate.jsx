...
const LoanPaymentCreate = () => {
    ...
    try {
      ...
      setFormData(prev => {
        ...
        if (!newForm.categoryId) {
          const defaultCat = catData.find(c => String(c.name).trim().toUpperCase() === 'LOON DEWA');
  ...
  const handleAddCategory = async (name) => {
    const created = await accountingService.createExpenseCategory({ name: name.trim() });
    const row = { ...(created || {}), id: created?.id || created?.uuid, name: created?.name || name.trim() };
        ...
        <div className="premium-body" style={{ background: 'white' }}>
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-row">
              ...
              <div className="form-col">
                <div style={{ display: 'flex', border: '1px solid #93c5fd', borderRadius: '6px', overflow: 'hidden', background: 'white', alignItems: 'center' }}>
                  ...
                  <input type="text" name="note" placeholder={t("Payment Description in a short note")} value={formData.note} onChange={handleChange} style={{ flex: 1, padding: '12px 16px 12px 0', border: 'none', outline: 'none', fontSize: 'var(--fs-14, 14px)' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '32px' }}>
              ...
              <button type="button" className="btn-danger" onClick={() => navigate('/loan/payment')} style={{ background: 'var(--danger)', padding: '10px 24px', fontSize: 'var(--fs-14, 14px)', borderRadius: '4px' }}>
                {t("Close")}
              </button>
            </div>
          </form>
