...
const LoanReceiveCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(t("Loan Receive added successfully!"));
      navigate('/loan/receive');
    } catch (error) {
      console.error("Error creating loan receive:", error);
      toast.error(t("Failed to create loan receive: {{v0}}", { v0: error?.message || t("server error") }));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="dashboard-content" style={{ paddingBottom: '100px' }}>
      ...
      <div className="premium-card">
        <div className="premium-header">
          <h2 className="premium-title">{t("Create Loan Receive")}</h2>
